...
export class KidComponent implements OnInit {
  ...
  ngOnInit(): void {
    const kid_rev = <HTMLDivElement>document.querySelector('#kid-rev');

    let querykids = this.firestore.collection('kid');
    let querywish = this.firestore.collection('wishlist');
    let querycart = this.firestore.collection('cart');

    function renderkid(doc: any){
      ...
      button1.addEventListener('click', (e: any) => {
        ...
        querykids.doc(id).ref.get().then((querysnapshot) => {
          ...
          if (test == true){
            ...
            querykids.doc(id).update({wish: true});
          }
        })
      });
    ...
    querykids.ref.get().then((querySnapshot) => {
